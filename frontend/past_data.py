...
def display_past_data():
    st.markdown("<h1 style='color:#00A3E0;'>Project History</h1>", unsafe_allow_html=True)
    ...
    st.markdown('<div class="main-section">Data Highlights</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-section">Functional Components</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="sub-section">Effort Overview</div>', unsafe_allow_html=True)
    st.markdown('<div class="highlight-section">', unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    with col1:
        st.markdown(f'<div class="low">Low: €{low_value:.2f}</div>', unsafe_allow_html=True)
    with col2:
        st.markdown(f'<div class="average">Average: €{average_value:.2f}</div>', unsafe_allow_html=True)
    with col3:
        st.markdown(f'<div class="high">High: €{high_value:.2f}</div>', unsafe_allow_html=True)
    ...
    fig = px.scatter_matrix(df, dimensions=['AFP', 'Effort', 'Duration'],
    ...
    fig = px.histogram(df, x='Effort', nbins=10, title='Distribution of Effort',
                       color_discrete_sequence=['#00A3E0'], template='plotly_white')
    fig.update_layout(
        title_font=dict(size=16, color='#00A3E0', family='Arial, sans-serif'),
    ...
    fig = px.histogram(df, x='Duration', nbins=10, title='Distribution of Duration',
                       color_discrete_sequence=['#00A3E0'], template='plotly_white')
    fig.update_layout(
        title_font=dict(size=16, color='#00A3E0', family='Arial, sans-serif'),
        xaxis_title='Effort (hours)',
