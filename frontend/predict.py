...
def display_predictive_analysis():
    ...
    with st.form("estimation_form"):
        ...
        data_communication = st.selectbox('📶 Data Communication', influence_choices,
                                          help="Assess the requirement for data communication in the system")
        distributed_processing = st.selectbox('🌐 Distributed Data Processing', influence_choices,
                                              help="Evaluate if the system requires distributed processing capabilities")
        performance = st.selectbox('⚡ Performance', influence_choices,
                                   help="Indicate the significance of performance constraints in the project")
        use_configuration = st.selectbox('⚙️ Heavily Use of Configuration', influence_choices,
                                         help="Determine if extensive configuration is needed for the application")
        transaction_rate = st.selectbox('📅 Transaction Rate', influence_choices,
                                        help="Select the frequency of transactions expected in the system")
        online_data_entry = st.selectbox('🖱️ Online Data Entry', influence_choices,
                                         help="Assess whether interactive data entry is required for users")
        user_efficiency = st.selectbox('💡 End-User Efficiency', influence_choices,
                                       help="Indicate the importance of user efficiency in the application.")
        online_update = st.selectbox('🔁 Online Update', influence_choices,
                                     help="Evaluate the need for interactive updates within the system.")
        computational_complexity = st.selectbox('🧩 Computational Complexity', influence_choices,
                                                help="Assess whether the system has high computational complexity.")
        reusability = st.selectbox('🔄 Reusability', influence_choices,
                                   help="Determine the reusability of components in the project.")
        installation_ease = st.selectbox('📦 Installation Ease', influence_choices,
                                         help="Indicate how important ease of installation is for the system.")
        operation_ease = st.selectbox('🛠️ Operation Ease', influence_choices,
                                      help="Evaluate how critical ease of operation is for end users.")
        multiple_sites = st.selectbox('🌍 Multiple Sites', influence_choices,
                                      help="Assess whether the system will be deployed across multiple locations.")
        ease_of_modification = st.selectbox('✏️ Facilitate Change', influence_choices,
        ...
        if submitted:
            ...
            if response.status_code == 200:
                ...
                predicted_duration, predicted_cost = result

                col1, col2, col3 = st.columns(3)
                col1.metric(label="Cost", value=f"€{predicted_cost:.2f}")
                col2.metric(label="Duration", value=f"{predicted_duration} hours")
                col3.metric(label="AFP", value=f"{predicted_duration:.2f}", help="Adjusted Function Points")
                ...
                report_response = requests.post("http://0.0.0.0:8000/report", json=input_data)
