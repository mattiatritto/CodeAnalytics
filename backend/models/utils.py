...
def predict_duration_and_costs(inputs: InputModel):
    ...
    input_array = pandas.DataFrame([[afp, float(inputs.effort), float(inputs.ilf_count),
                             float(inputs.ei_count), float(inputs.eo_count),
                             float(inputs.eq_count), float(inputs.eif_count)]])
